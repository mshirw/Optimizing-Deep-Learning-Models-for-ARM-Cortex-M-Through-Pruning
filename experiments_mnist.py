...
def Generate_h(model, output_shift_dict, channels_list, filename):
    print("--- Output weight ---")
    conv_output_channel = [channels_list[0], channels_list[1]]
    count = 0
...
file = "./saves/mnist_experiments/mnist_model_best_"
...
for convlayer in range(len(conv_list)):
    for percent in range(len(percent_list)):
        ...
        filename = "./saves/mnist_experiments/cortexm_weight_"
        filename = filename + str(channels_list[0]) + '_' + str(channels_list[1]) + '_' + str(channels_list[2]) + '_' + conv_list[convlayer] + '_' + percent_list[percent] + ".h"
        # cortexm_weight_0_8_256_conv1_20.h
