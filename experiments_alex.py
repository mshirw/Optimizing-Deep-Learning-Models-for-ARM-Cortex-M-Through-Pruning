...
from net.quantization import weight_quantization, Quantize, Quantize_2, FracBits, weight_quant_perchannel, Shift_Right_Bits, ShiftRight, Asymmetric_quantization, Quantize_torch, FracBits_torch
...
def test(model, quant=False):
    ...
    with torch.no_grad():
        for data, target in test_loader:
            ...
            # output = model(data)
            output = alex_quant_activate_inference(model, data)
...
def alex_quant_activate_inference(model, data):
    x = data
    conv_count = 0
    fc_count = 0
    global batch_count
    ...
    for i in range(len(model.feature)):
        x = model.feature[i](x)
        """
        if isinstance(model.feature[i], nn.MaxPool2d):
    ...
    for j in range(len(model.classifier)):
        ...
        # print("fc:", x.size())
        if isinstance(model.classifier[j], nn.Linear) or isinstance(model.classifier[j], MaskedLinear):
            shift_right_bit = Shift_Right_Bits(x.cpu().detach().numpy().astype(int))
        ...
        if isinstance(layer, nn.Sequential):
            for i in range(len(layer)):
                if isinstance(layer[i], nn.Conv2d):
                    channels_list.append(layer[i].weight.data.size()[0])
                elif isinstance(layer[i], nn.Linear):
                    channels_list.append(layer[i].weight.data.size()[1])

    return channels_list

def Generate_h(model, output_shift_dict, channels_list, filename):
    print("--- Output weight ---")
    ...
    weight_name = "classifier.0.weight"
...
file = "./saves/alex_experiments/pruned_model_best_"

for convlayer in range(len(conv_list)):
    for percent in range(len(percent_list)):
        modelname = file + conv_list[convlayer] + '_' + percent_list[percent] + ".ptmodel"
        print(modelname)
        ...
        filename = "./saves/alex_experiments/cortexm_weight_"
        for i in range(len(channels_list)):
            filename = filename + str(channels_list[i]) + '_'
        filename = filename + conv_list[convlayer] + '_' + percent_list[percent] + ".h"
        Generate_h(model, output_shift_dict, channels_list, filename)
